...
if (Modernizr.webgl) {
  ...
  function ready(error, options) {
    ...
    fetch("https://raw.githubusercontent.com/theojolliffe/healthindexlads/main/E92000001.json")
    .then(res => res.json())
    .then(json => {
      england = json;
    });
    ...
    function loadAreaHealth(code) {
      ...
      fetch("https://raw.githubusercontent.com/theojolliffe/healthindexlads/main/"+selected.parent+".json")
      .then(res => res.json())
      .then(json => {
        region = json;
      });
      ...
      .then(json => {
        ...
        subDomains = priorities.filter(e => {
...
`
    ...
    function getCodes(myArea) {
      ...
      setTimeout(function(){
        let found = find("Rank", subDomains.map(d => d.hlRankType))
        for (var i = 0; i < 4; i++) {
          if (found.includes(i)) {
            beeswarm(i)
          }
